...
REQ_TYPE = "json"
SERVICE_NAME = "LOCALDATA_010106"
CHUNK_SIZE = 1000


# 단일 페이지 조회
def pharmacy_data_load(page: int, per_page: int):
...
# 모든 페이지 데이터 조회
def all_pharmacy_data(per_page=CHUNK_SIZE, max_workers=10):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(pharmacy_data_load, page, per_page) for page in range(1, total_pages + 1)]
...
def save_to_csv(data: list, output_path: str = "data/raw/pharmacy__raw.csv"):
...
if __name__ == "__main__":
    pharmacy_data = all_pharmacy_data()
